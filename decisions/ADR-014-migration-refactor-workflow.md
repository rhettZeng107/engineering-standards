...
3. Terra Medium 只用于契约已锁、边界明确的机械编码与补测试；除此之外的实现、分析、联调、评审或跨契约工作不路由到 Terra。
4. 跨文件实现、常规前后端联调、编译错误修复、契约锁定后的非机械实现及普通最终主 CR 使用 Sol High。
5. 跨多仓契约、架构、DB schema、鉴权、生产、外部集成、高影响非确定性争议及相应高风险复审使用 Sol xhigh。
6. Luna 不用于企业项目；任何模型判断均不得替代 build/test/LSP/DB/API/E2E/git 等运行证据。

本修订覆盖 2026-08-19 修订中 Terra critic / Sol High 的粗粒度路由，不改变机器门、完整契约、选择性两票和最终 CR 边界。
