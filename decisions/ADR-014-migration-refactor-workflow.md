...
6. 模型与上下文按成本路由：确定性检查用脚本；Critic、Skeptic、合同锁和最终 CR 使用 Sol；Terra 不承担分析或评审；禁止默认 full-history fan-out。

本修订覆盖本 ADR 2026-07-15 硬门收口中“最后连续两轮 dry”和“severity 单独触发投票”的旧口径，其余规范化合同、哈希锁、字段门和完整范围铁律保持不变。

## 修订(2026-08-31)— 模型按任务复杂度与风险路由

本节路由已于2026-09-07按涛哥拍板更新。此前模型选择仅属历史；当前统一入口为[Harness标准的模型路由](../standards/provider-neutral-ai-coding-harness-standard.md#model-and-reasoning-routing)。独立Reviewer/critic agent是否调用遵循[ADR-036当前决策](./ADR-036-code-review-mandatory-gate.md)，完整性机器门与高影响多证据审查继续保留。

**问题**:统一高推理强度会让机械编码、补测试和确定性验证承担不必要等待；反过来，把跨多仓契约、架构、DB、鉴权或外部集成降档又会增加误判与返工。

**决策**:

1. `rg`/LSP/build/test/格式、哈希和清单校验等确定性工作优先工具或脚本。
2. 简单可逆任务由当前主会话直接完成，不为降低模型档位额外启动子任务。
3. 复杂任务使用`gpt-6-astra high`；其它任务使用`gpt-5.6-sol high`。复杂度依据依赖和不确定性，不按文件数判断。
4. 常规且边界清楚的实现/修复/测试/文档使用Sol；复杂任务中的独立常规子任务也可使用Sol。
5. 跨仓契约、架构、DB/鉴权/生产风险、迁移完整性、外部集成、难复现诊断及全局Harness调整使用Astra High；不自动升xhigh/max/ultra。启动profile与运行中模型选择分开验证。
6. Luna 不用于企业项目；任何模型判断均不得替代 build/test/LSP/DB/API/E2E/git 等运行证据。

本修订覆盖历史Terra/Sol固定路由，保留机器门、完整契约、多证据反例检查和最终diff自查。模型升级不是省略验证的依据。
...
| 2026-08-31 | 修订 | 模型按复杂度与风险路由：工具处理确定性工作；Terra Medium 仅机械编码/补测试；Sol High 负责跨文件实现/联调/编译修复/普通 CR；Sol xhigh 负责高风险分析与复审。 |
