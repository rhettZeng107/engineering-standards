...
> **2026-09-07修订**：本地验证按实际变更影响选择，页面/契约必须真实业务验收，迁移保留E1/E2；不因标准轨重复无关上传/鉴权/CRUD，也不因简单轨跳过受影响行为。CI保留项目配置的检查，本地与CI证据仅在候选、依赖、环境和覆盖适用时复用；不同运行环境不可互相冒充。失败先自治修复，真实阻塞再升级。见[ADR-039](./ADR-039-standard-task-workflow.md)，本页历史全量/固定审批安排冲突处不再执行。
