...
执行顺序：编码中定向验证 → stage最终候选 → 主会话一次汇总diff、关联代码及有效验证证据 → commit → 独立命令验证git状态。证据复用与失效、影响回归、批次提交及首次集成验收观察遵循[ADR-039](./ADR-039-standard-task-workflow.md)。CR不重复有效命令；修复后定向复查，共享基础/契约/环境变化扩大验证。保留密钥扫描、完整业务契约、鉴权四条、生产/破坏性操作授权和推送边界。移除固定评审agent不代表移除反例分析或运行验证。
