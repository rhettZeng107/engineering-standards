...
> **2026-09-07修订**：普通改动复用一份任务记录承载范围、实现和验收，不因文件数量强建Spec/Plan/Tasks；迁移、架构或复杂长任务保留必要分层，禁止重复记录。归档不增加已授权批次的形式审批。执行与试行口径见[ADR-039](./ADR-039-standard-task-workflow.md)，本页旧固定文档安排冲突部分不再执行。
