...
- Build/Test/制品可以在Runner容量内并发。部署按共享资源而非整台主机一刀切：同一应用、数据库迁移、网关及共享基础配置必须串行；不同应用只有在回滚边界独立且目标主机有显式信号量时才可有限并行。
...
部署态 E2E 使用固定、最小权限、可审计的 CI 账号池，不复用产品负责人、开发人员或演示账号。只读Job按并发槽分配隔离账号；写入Job还必须使用独立账号、每次生成唯一业务键并清理测试数据，只对确实共享的业务资源取模块锁。无法证明账号隔离、唯一数据或资源边界时保持串行。凭据只从仓库/组织 Secret 注入，账号、角色或权限矩阵变化必须由身份真理源回读，不能只改测试脚本。若现行角色模型无法表达严格最小权限，应记录结构性限制，不得把受限菜单的管理员账号描述成最小权限。

账号隔离不等于认证入口可无限并发。启用只读E2E并行前必须实证认证端点的每账号、每IP及全局限流窗口，并按单Job最坏登录次数计算认证预算；超过预算的认证重模块使用模块级队列或错峰，普通低登录频率模块仍可占用账号槽并行。不得通过提高或关闭生产同口径安全限流来换取测试吞吐，也不得把429后由重试转绿记为无抖动并发通过。
...
- Job缓存统一放在`${{ runner.temp }}`下：NuGet、npm、pnpm分别使用`nuget-packages`、`npm-cache`、`pnpm-store`目录。缓存键至少包含目标SDK或Node/包管理器主版本，以及全部相关锁文件、`csproj/props/targets/global.json/NuGet.config`哈希。恢复旧前缀仅用于内容寻址包缓存，最终安装仍必须使用锁文件和`--frozen-lockfile`/等价确定性门。
- 不缓存`node_modules`、`build/dist`、测试结果或Playwright浏览器目录。浏览器与包管理器本体应进入固定Runner Job镜像并由Docker层复用；业务依赖仍由Cache Server按锁文件键管理。受限网络可把可信国内镜像加入直连清单，境外源走受控代理，但镜像/代理不能替代锁文件、hash键和冷缓存回退验证。
...
- 同一应用的替换部署始终互斥；应用锁覆盖build image、compose replace、health和失败回滚，不能只锁上传。
- 不同应用只有在Compose/容器/回滚基线互不覆盖时才可并行，并必须受目标主机级信号量限制。初始上限从2路开始，以CPU、内存、磁盘IO和回滚演练决定是否调整；第三路必须排队。
- 数据库迁移使用跨应用独立全局锁；APISIX/网关、共享认证配置及其他共享基础资源各使用命名资源锁。一个部署同时需要应用锁、主机槽和资源锁时，所有项目域必须采用一致的获取顺序，避免死锁。
...
- [ ] 应用锁、主机部署信号量及数据库/网关资源锁均已实跑；同应用和第三路排队、跨应用有限并行、失败回滚隔离有反例证据
- [ ] 精确 SHA、容器 revision、网关 smoke和部署态定向 E2E 已闭环
- [ ] JUnit/TRX/截图/hash制品已下载回读
- [ ] Gitea 成功后 GitHub 镜像已对账；ADO 已转历史只读且未再触发
- [ ] 只读E2E使用隔离账号槽并已按认证限流预算并发验证；认证重模块有模块级队列/错峰；写入E2E使用独立账号、唯一业务数据和模块资源锁；凭据仅来自Secret
...
- [Docker Build cache optimization](https://docs.docker.com/build/cache/optimize/)：固定基础层、外部缓存与缓存失效边界。
- [Playwright CI](https://playwright.dev/docs/ci)：固定浏览器镜像、CI依赖与并行执行建议。
