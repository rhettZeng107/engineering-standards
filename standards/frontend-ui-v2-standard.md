...
- 宿主侧栏、菜单分组、搜索、账户区与顶栏也须跟随实际主题；浅色模式使用浅色导航工作面与深色文字，深色模式使用深色导航工作面与浅色文字。品牌色用于选中、焦点和动作，不把侧栏固定为深蓝色。
...
6. 列表视觉顺序为「标题/说明 → 带标签的字段筛选及右侧重置、查询 → 左侧结果数/排序说明与右侧刷新、密度、列设置 → 表头/数据/分页」。宽屏常用筛选项可排四列，窄屏依次收为两列、一列；三个工具按钮统一 40×40px 边框盒，图形约 18×18px 且居中，不得出现小图标配大空框或受裁切、变形的图形；同时须有可辨识名称、提示、键盘焦点及适合触控的命中区。视觉示例见[明暗列表页](../references/ui-v2-theme-examples/list.html)，交互与响应式细则以列表标准为准。
...
- 单页面参数配置属于独立业务路由页。按配置范围、业务分区、影响预览、修改原因及统一保存组织；长页面显示未保存状态和当前影响，保存失败保留输入并定位错误。实际字段、权限、生效时点与历史快照由所属业务契约决定。明暗布局示例见[参数页](../references/ui-v2-theme-examples/settings.html)。
...
- 视觉检查：桌面及适用移动端在 100%、125%、150% 缩放下无重叠、截断和无效大留白；固定列、横向滚动和操作区可用。列表三工具在真实浏览器中分别核对 40×40px 边框盒、约 18×18px 的居中图形和刷新、密度、列设置原有功能。
- 主题检查：浅色、深色及跟随系统的实际画面均核对宿主侧栏、菜单分组、搜索、账户区、画布、表格、表单、弹窗、通知和状态徽章；浅色导航须为浅色工作面，深色导航须为深色工作面。切换不清除查询或未保存输入，不刷新已打开的子应用页签。正文与背景对比度至少 4.5:1，状态文字、图标、边界至少 3:1；颜色以外仍有文字或图形语义。
