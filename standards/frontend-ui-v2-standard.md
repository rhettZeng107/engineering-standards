...
6. 列表视觉顺序为「标题/说明 → 带标签的字段筛选及右侧重置、查询 → 左侧结果数/排序说明与右侧刷新、密度、列设置 → 表头/数据/分页」。宽屏常用筛选项可排四列，窄屏依次收为两列、一列；三个工具图标须有可辨识名称、提示、键盘焦点及适合触控的命中区。视觉示例见[明暗列表页](../references/ui-v2-theme-examples/list.html)，交互与响应式细则以列表标准为准。
