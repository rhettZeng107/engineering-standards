...
子应用发布 BP = **菜单经 manifest 自动进 SYS(禁手工种库)+ API按SYS OIDC Discovery/JWKS验签+前后端经Gitea Actions部署不可变容器到容器平台和统一网关+部署后定向E2E验证真实业务数据**。业务用户从独立`/bp/#/login`入口登录；Standard档在真实操作空闲未达4小时前由BP续发短期Access，嵌入子应用需通过Bridge报告真实操作，不能靠轮询续会话。完整执行合同见 [MOM OIDC 统一认证与容器化部署详细手册](mom-oidc-containerized-delivery-guide.md)及[Gitea Actions内网容器CI/CD标准](gitea-actions-onprem-container-cicd-standard.md)。
...
|---|---|---|---|---|
| 1 | **前端三件套 + Bridge/容器适配** | `routes.config.mjs`→`generate-manifest.mjs`产manifest；BrowserRouter `basename`；Bridge V1只在内存接收OIDC Access Token/PlantCode，禁URL JWT；Standard业务槽真实操作须报告`subapp-user-activity` | `subapp-onboarding-guide`步骤1-8/附录O · OIDC容器手册§2.4/§4-5 | `pnpm build`菜单数一致；Bridge及活动信号单测；本地容器冒烟 |
