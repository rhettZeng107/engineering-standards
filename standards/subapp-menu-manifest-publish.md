...
- `MenuController` `[Route("<app>api/[controller]")]` `GET manifest` `[AllowAnonymous]` 读 wwwroot(prod 缺文件 fail-fast)
...
| 2 | **console 菜单泄漏 BP 404** | BP `BuildBpMenuAsync` 仅按 `AppName+BShow` 过滤,**漏 PortalScope** → 老 console 菜单(BShow=true)成孤儿顶层显示、点击 404。修:BP 菜单查询加 `PortalScope='bp'` 过滤,或老 console 菜单 BShow=false 下线 |
...
- [ ] BP 无 console 菜单泄漏(PortalScope 隔离)
