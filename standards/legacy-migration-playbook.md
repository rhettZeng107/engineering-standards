...
| **源仓分支范围**(规则1,2026-06-22 涛哥定) | 迁移源**只取 master/develop**;customer 默认排除 | develop>master 取功能更全者 |

> 其他项目(如 HC)按自身技术栈声明基线,本手册方法不挑栈。

> **源仓分支范围铁律(规则1,ADR-028 修订 2026-06-22,TPM 设备手册漏迁复盘)**:迁移源仓库基线**只取 `master` 与 `develop`**(并存按项目声明谁优先 —— 默认取功能更全者);**`customer/*` 等客户定制分支默认不作迁移源参考基线**(客户特化≠主线功能),**除非涛哥显式要求**纳入。clone 后**必 `git for-each-ref --sort=-committerdate refs/remotes` 列分支活动度**核主线为最新;若 `customer/*` 远新于 master/develop(暗示主线停更、功能在客户线演进)→ **报涛哥拍板**,不擅自纳入、也不默认沿用停更主线。**多版本仓(FW vs Core 等并存)选基线必 diff 实证谁功能更全,禁靠单点假设**(踩坑:TPM 凭「FW 更全」假设选 FW,实测 net8 的 CORE 版活更久 / `_legacy` 6 仓 customer/kd/prd@2026-02 从未核)。
